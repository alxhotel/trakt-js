...
var base_url = 'api-v2launch.trakt.tv';
...
var getRequest = function (action, opts, options, callback) {
	...
	request_options.headers = {
		'trakt-api-key': '92cdc14d6b46b5eab2e670b9fe21f21c888408b4f73166b737a828bbdc2c1c33'
	};
...
var getGetUrl = function (action, opts, format, options) {
	...
	for (var i = 0; i < length; i++) {
		...
		if (options[param.name]) {
			if (typeof options[param.name] != 'string') options[param.name] = 'seasons/' + options[param.name]
			url += '/' + options[param.name].replace(/ /g, '+');
			if (opts.method == "summary") url += '?extended=full';
			if (opts.method == "images") url += '?extended=images';
		} else {
			if (!param.optional) {
				return undefined;
			} else {
	...
	if (opts.method == "season")  url += '/episodes?extended=full';
